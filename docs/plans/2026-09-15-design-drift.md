...
- **M2 — polish after the M1 walk-through** (owner 12:55, "Go!"; branch `feat/design-drift-m2`, D6/D7/D8 apply; two sonnet dispatchers in parallel on disjoint files; gates: typecheck once, oxlint changed files, vitest today+calendar, hub visual straight to `--update-snapshots=all`)
  - Done 13:28: `d3751de` (W1), `fea1238` (T1), `cbe902c` (K1+K2 + baselines); merged ff to main, deployed. Agy M2 total ≈ $0.37 (19 runs), Claude ≈ 424k (2 dispatchers, 1 reviewer, 2 investigators). All-day strip not in any seeded baseline — verify on the wall.
  - [x] **W1 — weather location form**: `setWeatherLocationAction` (`modules/weather/actions.ts`) has no UI caller, so weather never had an input. New `modules/weather/ui/weather-location-form.tsx` (client, `useActionState`, fields latitude/longitude/label, "Gebruik mijn locatie" via `navigator.geolocation`, empty latitude = weather off), mounted under `HubDisplayForm` on `(app)/settings/page.tsx`, `settings.weather.*` keys in nl/en.
  - [x] **T1 — all-day strip**: filter `allDay` out of the timeline rows in `today-timeline.tsx`; render them as one chip strip (`today-allday-strip.tsx`, glyph + title, no rail, no time) above the timeline card on hub and app. Birthdays therefore leave the timeline.
  - [x] **K1 — one member filter on `/hub/kalender`**: wrap the kalender page in `TodayFilterProvider`; `CalendarShell surface="hub"` reads `useTodayFilter()` and drops its own face row; the header `TodayHeaderFaces` is the only filter.
  - [x] **K2 — light grid blocks**: `EventChip variant="block"` fills with the light surface (`MEMBER_COLOR_CLASSES[color].surface` when an owner is known, else `palette.surface`), ink text, 4px left rail in `.dot`/`palette.solid`; matches `docs/design/calendar.md` (`rgba(93,95,239,0.06)` wash + dot). Same for `mobile-month-view.tsx`/`day-agenda-row.tsx` only if they use `solid` as a fill, not as a dot.
  - K2 finding (13:30): the "dark calendar blocks" were an M1 regression, not old code — `5e4c08d` put `MEMBER_COLOR_CLASSES[color].line` (a `bg-member-*-lijn` class) in the block's border slot, and that background beat `palette.surface`; `ce54284` baked it into the baselines. Fix is `.border` (`border-member-*-lijn`), baselines regenerated in run 4.
  - Gate fix (13:20): `calendar-shell.tsx` (client) importing the `@/modules/today` barrel pulled `server-only` into `calendar-shell-hub.test.tsx`; the lint rule forbids deep module imports, so `today-filter-context.tsx` moved to `src/components/hub/` (6 specifier edits, $0.10). Review 1🟡: weather form under the Calendars heading → own `SettingsSection id="weather"` ($0.02).
  - Header consistency (four header variants: `TodayHeader`, `routines-page-board.tsx:179`, `RoutinesBoard`, `store/page.tsx:77`) → M3.
...
| i6 | hub weather + header audit + all-day sites | sonnet (cavecrew-investigator) | 41k | 29 | done: weather slot intact, no location UI; 4 header variants; all-day unfiltered |
| d2a | M2 dispatcher W1+T1 (+ 🟡 fix) | sonnet (general-purpose) | 123k | 64 | done; 9 agy runs 178k in / 6k out / 238k cache ≈ $0.17 |
| d2b | M2 dispatcher K1+K2 (+ gate fix) | sonnet (general-purpose) | 126k | 61 | done; 10 agy runs 215k in / 7k out / 272k cache ≈ $0.20 |
| r-m2 | review M2 diff | sonnet (cavecrew-reviewer) | 53k | 27 | done: 1🟡 (weather section) |
| i7 | why hub blocks paint solid | sonnet (cavecrew-investigator) | 81k | 50 | done: `.line` bg class in border slot, M1 regression |
