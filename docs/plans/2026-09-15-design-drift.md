...
- **M2 — polish after the M1 walk-through** (owner 12:55, "Go!"; branch `feat/design-drift-m2`, D6/D7/D8 apply; two sonnet dispatchers in parallel on disjoint files; gates: typecheck once, oxlint changed files, vitest today+calendar, hub visual straight to `--update-snapshots=all`)
  - [ ] **W1 — weather location form**: `setWeatherLocationAction` (`modules/weather/actions.ts`) has no UI caller, so weather never had an input. New `modules/weather/ui/weather-location-form.tsx` (client, `useActionState`, fields latitude/longitude/label, "Gebruik mijn locatie" via `navigator.geolocation`, empty latitude = weather off), mounted under `HubDisplayForm` on `(app)/settings/page.tsx`, `settings.weather.*` keys in nl/en.
  - [ ] **T1 — all-day strip**: filter `allDay` out of the timeline rows in `today-timeline.tsx`; render them as one chip strip (`today-allday-strip.tsx`, glyph + title, no rail, no time) above the timeline card on hub and app. Birthdays therefore leave the timeline.
  - [ ] **K1 — one member filter on `/hub/kalender`**: wrap the kalender page in `TodayFilterProvider`; `CalendarShell surface="hub"` reads `useTodayFilter()` and drops its own face row; the header `TodayHeaderFaces` is the only filter.
  - [ ] **K2 — light grid blocks**: `EventChip variant="block"` fills with the light surface (`MEMBER_COLOR_CLASSES[color].surface` when an owner is known, else `palette.surface`), ink text, 4px left rail in `.dot`/`palette.solid`; matches `docs/design/calendar.md` (`rgba(93,95,239,0.06)` wash + dot). Same for `mobile-month-view.tsx`/`day-agenda-row.tsx` only if they use `solid` as a fill, not as a dot.
