...
Phase 1 (foundation, worktree): indigo `#0040e0` won the stitch-sidecar contradiction 24:1; tokens/fonts/icons/shell landed clean, gates green first pass. Phase 2 (6 parallel surface agents) review: 40 findings, 1 blocker + 8 required majors, all fixed: (blocker) social-onboarding double submit could mint duplicate households — now advisory-lock claim inside the transaction, race reproduced red/green in integration; (majors) Google sign-in stored a potentially calendar-scoped OAuth token in plaintext — sign-in path now persists no OAuth tokens at all (account-hook nulling) plus encryptOAuthTokens; swallowed signOut failure looping onboarding; removed members mistaken for social first-runs (former_member tombstone, migration 0015, session revocation); member-day-grid promoted unmatched members' private events into the shared household band (privacy) and the shared band blocked clicks (in-flow lane now); post-drag synthetic click reverted reschedules; frozen NOW-hero countdown (30s client tick + boundary refresh); settings nav rows broke accessible names. E2E round surfaced a real production bug: SlotPortal read the DOM during hydration render — view pill and FAB were invisible (useSyncExternalStore fix); plus WCAG AA contrast fixes (tabs alpha-composite, hub step-row opacity), deterministic clock pinning via MutationObserver, and a UTC-vs-family-timezone date-key flake fixed across 7 test files. Final proof: typecheck/lint/format clean, 831 unit + 331 DB-backed integration pass, visual baselines regenerated 42/42, full Playwright 193 pass with only the two documented environmental google-settings failures (machine has GOOGLE_CLIENT_ID set; spec asserts the unconfigured state). Carry-forwards: kiosk-audit asserts only body scrollHeight (internal pane overflow unasserted); FR22 fan-out still awaited in-request; google-settings spec needs an env-isolated runner (RESOLVED — see CLAUDE.md E2E gotchas: move `.env.local` aside before running e2e); account linking for social sign-in deliberately refused (unverified-email takeover risk) — revisit if wanted. Owner tasks open: add redirect URI `https://kynite.app/api/auth/callback/google` in Google console; visual sign-off on kynite.app.

## M21 — Google Calendar sync correctness (post-parity fix)

- [x] Status
- **Scope:** Fix two owner-reported production bugs in Google Calendar sync: (1) Google's "working location" cards were rendering as calendar events; (2) recurring events were showing up twice on the calendar. Both are correctness bugs in the sync engine, not new feature work.
- **Root causes:**
  - Working-location ghosts: the intended repair path was a tombstone sweep that would delete stale/unsupported event types on the next sync, but that repair was unreachable in practice — Google's `syncToken` gives incremental deltas, so an event type filtered out client-side never resurfaces in a later delta to be swept; the read side was papering over this with a runtime type-filter fallback instead of fixing ingestion.
  - Double-rendered recurring events: `originalStartTime` was discarded on ingest, so a recurring instance that Google sends as a modified exception (bearing `recurringEventId` + `originalStartTime`) was stored with no link back to which occurrence of the master it replaced. Google does not send an `EXDATE` on the master series for these exceptions, so `expandSeries` kept generating the original occurrence from the master's RRULE *and* the app stored the exception separately — both rendered.
- **Fix:** `worktree-agent-aaf4fa46a440ba865`, commits `d690061` (dedupe imported recurring series and clear stale status entries) and `85f316c` (backfill override slots at the source, not by guessing on read). Ingestion now backfills exception/override slots at write time keyed off `originalStartTime` instead of the read path guessing at series membership; unsupported event types (working-location, etc.) are dropped and swept correctly since the fix addresses ingestion, not just the read-side filter.
- **Review verdict:** BLOCK then APPROVE. Two HIGH findings blocked the first pass: (1) the read-side type-filter fallback was still present alongside the new ingest-time fix, masking whether the real fix worked and leaving a second, divergent source of truth for "what is a real event" — deleted, ingestion is now the single point of truth; (2) sync's own echo-suppression (writes the app makes locally get echoed back by Google's next sync) had no backfill path, so existing rows written before the fix would never get corrected without a resync — an echo-suppression-safe backfill was added so already-stored rows are corrected in place rather than requiring a full re-link. Also fixed in the same round: all-day event DST day-key bug (day-key computation for all-day recurring events broke across DST transitions in some timezones), a broken snapshot chain repair (sync's internal "last known good" snapshot could desync from the stored event set), and detach handling (turning a recurring exception into a fully standalone event) corrected. Migration honesty: migration 0018 is a one-time forced resync (not a data migration in the usual sense) — it exists to correct already-ingested bad data under the new ingest logic, and is documented as such rather than disguised as a schema change.
- **Deploy:** `railway up --service kynite --ci`; migrations 0017 (new column) and 0018 (one-time forced Google resync) ran as part of the release. Post-deploy verification: `GET /api/health` → `{"status":"ok",...}`; `GET /nl` → HTTP 200.
- **Gate:** `pnpm typecheck && pnpm lint && pnpm test:run` green on the merged tree (898 tests passed, 346 skipped, 63 files).
- **Non-blocking follow-ups left open** (not required for this fix to ship):
  - (a) All-day series expansion still no-ops west of UTC — the DST day-key fix corrected the immediate bug, but the proper fix is expanding all-day series in UTC inside `expandSeries` rather than in local time.
  - (b) The new backfill routes through the full upsert path, which can, in a narrow one-pass window, resurrect a locally-deleted override or overwrite an unpushed local edit — needs either a `!known.deletedAt` guard or a targeted (non-upsert) update instead.
  - (c) A detached exception (owner decision: keep it as a standalone event) still keeps `recurrence_parent_id` with `onDelete: cascade` — deleting the parent series deletes the detached event too, which contradicts "detached means independent."
  - (d) The drizzle-upsert conflict-branch semantics for the detach rule have no integration test coverage — unit-level only.
- **Known scoped-out limitation:** Google-cancelled instances that were never stored locally still render as ghost occurrences (the master's RRULE still generates them, and there's no tombstone to suppress them since nothing was ever written for that occurrence). Proper fix needs a tombstoned child insert on cancellation, not just deletion of already-stored rows. Left out of scope for this fix.

### Final e2e verification (2026-08-13)

Consolidated tallies, all runs env-isolated (`.env.local` moved aside per CLAUDE.md E2E gotchas), workers ≤2: app project — visual suites green after clock/date/greeting pinning (`f7d61fa`), incl. invite 5/5, settings 9/9, avatar-upload + sync-smoke 7 passed, earlier functional chunk 78 passed; hub 94 passed; share 16 passed; perf 5 passed (`--no-deps`). Zero unexplained failures.

### Owner TODO (manual, outside repo)
