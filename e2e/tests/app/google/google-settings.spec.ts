...
async function signUp(page: import('@playwright/test').Page): Promise<void> {
  // M17: the `app` project starts every context signed in as the baseline
  // parent (`playwright.config.ts` storage state), and the signed-in guard
  // sends that browser straight to /family. This spec wants its own household.
...
test.describe('google settings', () => {
  test('turns an anonymous OAuth start into a sign-in redirect', async ({ page }) => {
    // Anonymous means anonymous: the `app` project hands every context the
    // baseline parent's session (M17), and this assertion is about a browser
    // that has none.
    await page.context().clearCookies();
  ...
  test('explains that linking is switched off when credentials are unset', async ({ page }) => {
    ...
    // "Instellingen" is that hub now, and Google is one section on it. At the
    // `app` project's phone width (M17) that link lives in the overflow sheet
    // rather than the bar — which is the navigation a parent actually
    // performs, so it is the one under test.
