 ...
 *
 * `dateLabel`/`timeLabel` name the two halves *to the user*; `dateLabels` and
 * `timeLabels` are the halves' own internal copy (picker buttons, validity
 * messages) passed straight through. Two prop pairs rather than one flat bag
 * because they have different audiences, and only the first pair is something
 * a call site thinks about — the second is what the app's wrapper fills in.
 */
export type DateTimeFieldProps = {
  /** The household's convention, handed to both halves. */
  locale?: FormattingLocale;
  dateLabels?: Partial<DateFieldLabels>;
  timeLabels?: Partial<TimeFieldLabels>;
  name?: string;
