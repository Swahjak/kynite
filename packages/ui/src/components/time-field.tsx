 ...
 *
 * Like `DateField`, it is handed its convention and its copy rather than
 * reading either from context — see that file for why.
 */
export type TimeFieldLabels = {
  /** Accessible name of the trailing clock button, and of the option list. */
  pick: string;
  /** Validity message while the typed text cannot be read as a time. */
  invalid: string;
};

const DEFAULT_LABELS: TimeFieldLabels = {
  pick: 'Pick a time',
  invalid: 'Enter a time in the expected format.',
};

export type TimeFieldProps = {
  /** The household's clock convention; decides 12- vs 24-hour display. */
...
export function TimeField({
  ...
  React.useEffect(() => {
    ...
    if (!input) return;
    input.setCustomValidity(unreadable ? copy.invalid : '');
  }, [unreadable, copy.invalid]);
